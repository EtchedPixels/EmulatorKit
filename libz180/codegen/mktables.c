...
void generateCodeTable (FILE* opcodes, FILE* code)
{
	...
	{			
		...
		*q = 0;
		...
		/* Print function stub */
		fixName(line, name);
		fprintf(code, "static void %s (Z180Context* ctx)\n{\n", name);
...
/* ---------------------------------------------------------
 *  Parser structures. Not exactly the same as in z180.c
 * --------------------------------------------------------- */
typedef enum
{
	OP_NONE,
...
struct Z180OpcodeEntry
{
	int pre_skip;
	int post_skip;
	char* func;		/*	Z180OpcodeFunc* func;*/
	
	int operand_type;
	char* format;	
	
	struct Z180OpcodeTable* table;
};
...
struct Z180OpcodeTable
{
	char* name;
	int opcode_offset;
	struct Z180OpcodeEntry entries[256];
};
...
struct Z180OpcodeTable* createTableTree(FILE* opcodes, FILE* table)
{
	struct Z180OpcodeTable* mainTable;
	struct Z180OpcodeTable* current;
	char line[MAX_LINE];
	char tmp[MAX_LINE];
	char* cur;
	byte code;
	...
	mainTable = calloc(1, sizeof(struct Z180OpcodeTable));
	mainTable->name = strdup("");
	
	fprintf(table, "static const struct Z180OpcodeTable opcodes_main;\n");
	...
	do
	{
		...
		for (i = 0; i < prefixLen - 1; i++)
		{
			...
			if (current->entries[code].table == NULL)
			{
				...
				current->entries[code].table = calloc(1, sizeof(struct Z180OpcodeTable));
				current = current->entries[code].table;
				current->name = strdup(tmp);				
				fprintf(table, "static const struct Z180OpcodeTable opcodes_%s;\n", current->name);
...
void scanOpcodes(FILE* opcodes, struct Z180OpcodeTable* mainTable)
{
	char line[MAX_LINE];
	char name[MAX_LINE];
	char fmt[MAX_LINE];
...
void outputTable(struct Z180OpcodeTable* table, FILE* file)
{
	int i;
	struct Z180OpcodeEntry* opc;
	struct Z180OpcodeTable* tbl;
	char fmt[MAX_LINE];
	
	printf("Outputting table %s...", table->name);
	
	fprintf(file, "static const struct Z180OpcodeTable opcodes_%s = { %d, {\n", table->name, table->opcode_offset);
...
{
	struct Z180OpcodeTable* mainTable = createTableTree(opcodes, table);
