...
IN0 (A|B|C|D|E|F|H|L),\(n\)
	BR.%1 = ioRead(ctx, read8(ctx, ctx->PC++));
	RESFLAG(F_H | F_N);
	adjustFlagSZP(ctx, BR.%1);
	adjustFlags(ctx, BR.%1);
...
OUT0 \(n\),0
	ioWrite(ctx, read8(ctx, ctx->PC++), 0);

OUT0 \(n\),(A|B|C|D|E|H|L)
	ioWrite(ctx, read8(ctx, ctx->PC++), BR.%1);
...
#
TST (A|B|C|D|E|H|L)
	adjustLogicResult(ctx, 0, BR.A & BR.%1);

TST \(HL\)
	adjustLogicResult(ctx, 0, BR.A & read8(ctx, WR.HL));

TST n
	adjustLogicResult(ctx, 0, BR.A & read8(ctx, ctx->PC++));

TSTIO \(C\),n
	adjustLogicResult(ctx, 0, ioRead(ctx, BR.C) & read8(ctx, ctx->PC++));

#
# SLP - not modelled yet
#
SLP
	;
