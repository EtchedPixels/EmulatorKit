 ...
 *	Platform features
 *
 *	6509 processor card for rcbus with I/O at $FExx and 20bit addressing
 *	Motorola 68B50
 *	IDE at 0xFE10-0xFE17 no high or control access
 *	First 512K RAM Second 512K ROM (standard 512/512 cat set ROM high)
 *	RTC at 0xFE0C
 *	16550A at 0xFEC0
...
{
	uint32_t eaddr = (ramrom[mempage] & 0x0F) << 16;
	/* 0 and 1 are internal and unbanked */
	if (addr > 1)
		eaddr += addr;
	else
		eaddr = addr;
...
void write6502(uint16_t addr, uint8_t val)
{
	...
	/* IO does not currently decode the page bits */
	if (addr >> 8 == iopage) {
...
{
	fprintf(stderr, "rcbus-6509: [-1] [-A] [-a] [-f] [-i idepath] [-R] [-r rompath] [-w] [-d debug]\n");
	exit(EXIT_FAILURE);
}
...
int main(int argc, char *argv[])
{
	...
	if (input == 0) {
		fprintf(stderr, "rcbus-6509: no UART selected, defaulting to 16550A\n");
		input = 2;
	}
	...
	if (read(fd, ramrom + 0x80000, 0x80000) != 0x80000) {
		fprintf(stderr, "rcbus-6509: banked rom image should be 512K.\n");
		exit(EXIT_FAILURE);
	}
	close(fd);
	...
	init6502();
	reset6502();
	ramrom[0] = 0x0F;		/* 6509 initializes the bank reg to 0xF */
