...
struct sasi_bus {
    uint8_t control;		/* Control and status lines */
 ...
 */
static void sasi_done(struct sasi_disk *sd, uint8_t status)
{
    sd->status = status;
...
{
    sd->bus->control |= SASI_MSG|SASI_IO;
    return sd->status;
}
