...
{
	fprintf(stderr, "rcbus-68hc11: [-b] [-B] [-F] [-f] [-R] [-r rom] [-i idedisk] [-S sdcard] [-m monitor] [-w] [-d debug]\n");
	exit(EXIT_FAILURE);
}
...
int main(int argc, char *argv[])
{
	...
	if (rom == 0 && bank512 == 0 && bankhigh == 0 && bankflat == 0) {
		fprintf(stderr, "rcbus-68hc11: no ROM\n");
		exit(EXIT_FAILURE);
	}
	...
	if (rom) {
		...
		if (read(fd, ramrom + 32768, 32768) != 32768) {
			fprintf(stderr, "rcbus-68hc11: short rom '%s'.\n", rompath);
			exit(EXIT_FAILURE);
		}
		close(fd);
	}
	if (monpath) {
		...
		if (read(fd, monitor, 12288) != 12288) {
			fprintf(stderr, "rcbus-68hc11: short monitor '%s'.\n", monpath);
			exit(EXIT_FAILURE);
		}
		close(fd);
	}
	...
	if (bank512 || bankhigh || bankflat) {
		...
		if (read(fd, ramrom, 524288) != 524288) {
			fprintf(stderr, "rcbus-68hc11: banked rom image should be 512K.\n");
			exit(EXIT_FAILURE);
		}
		if (bank512 || bankhigh)
			bankenable = 1;
